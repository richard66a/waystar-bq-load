...
Use the prebuilt query pack in [../sql/validation_queries.sql](../sql/validation_queries.sql).
...
Uses [../sql/00_setup_all.sql](../sql/00_setup_all.sql) and replaces the `__GCS_URI__` placeholder.
...
Uses [../sql/06_scheduled_query_proc.sql](../sql/06_scheduled_query_proc.sql).
...
```sql
SELECT gcs_uri, originating_filename, processed_timestamp, rows_loaded, rows_expected, parse_errors, status
